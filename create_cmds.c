...
t_command	**create_simple_cmd(t_tools *shell)
{
	...
	i = 0;
	init_cmd(&cmd_list, &cmd, shell->tkn_list);
...
t_command	**create_adv_cmd(t_tools *shell)
 {
	...
	tkn = shell->tkn_list;                 									   // make our tmp token equal to the head of our token_list. In this example head tkn = "ls"
	while (j <= shell->number_of_pipes)                                   // 1 pipe present equals 2 commands and if i starts at 0 then we run our while loop for two rounds
	{
		...
		if (shell->number_of_redir > 0)                                              // if we have redirections logged 
			redir_init(shell, cmd);  // allocate space for redirection
	...
	//assign_literal() has replaced the below code for refactoring
	// cmd->args[i] = ft_calloc(ft_strlen(tkn->content), sizeof(char));
